   ...
   namespace
   {
                                    ...
                                    T const &value)
      {
         ...
         for (std::size_t idx = 0, len = value.buckets_.size(); idx != len; ++idx)
         {
            ...
            {
               fmt::format_to(std::back_inserter(buffer), "{}{{le=\"{}\" {}}} {}\n", bucket_label, label.labels, bucket,
                              bucket_count);
            }
         }
...
spider2::string spider2::prometheus::metrics_manager::format_metrics() const
{
   ...
   auto label_buffer = fmt::memory_buffer{};
   label_buffer.reserve(512);
   ...
   for (size_t i = 0, len = metrics_.size(); i != len; ++i)
   {
      ...
      std::visit(
          ...
          {
             if constexpr (std::is_same_v<std::decay_t<decltype(value)>, labeled_counter>)
             {
                const labeled_counter &counter_value = value;
                auto metrics_values = counter_value.collect();
                auto label_copy = label;

                for (auto &[counter_label, val] : metrics_values.second)
                {
                   label_buffer.clear();
                   if (label.labels.empty())
                   {
                      fmt::format_to(std::back_inserter(label_buffer), "{}=\"{}\"", metrics_values.first,
                                     counter_label);
                   }
                   else
                   {
                      fmt::format_to(std::back_inserter(label_buffer), "{},{}=\"{}\"", label.labels,
                                     metrics_values.first, counter_label);
                   }

                   label_copy.labels = std::string_view{label_buffer.data(), label_buffer.size()};

                   format_metrics_to_buffer(buffer, label_copy, counter::type, val);
                }
             }
             else
             {
